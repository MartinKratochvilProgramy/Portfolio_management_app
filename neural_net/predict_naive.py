...
#LOADS NEURAL NET AND PREDICTS CHANGES IN S&P-500 WHICH ARE THEN STORED IN 'predictions.csv'
...
def predict_naive():
    ...
    for i in range(BOX_SIZE - 1):
        val = df['Close'].iloc[0 - BOX_SIZE + i]
        if math.isnan(val):
            val = df['Close'].iloc[0 - BOX_SIZE + i - 1]
        if math.isnan(val):
            val = df['Close'].iloc[0 - BOX_SIZE + i - 2]
        input_vector.append([val])

    nn = Neural_net(6, (64, 128, 128, 64), 1)
    nn.load_model('neural_net/BOX_7_6(64, 128, 128, 64)1')
    ...
    print("Yesterday predicted: ", output_vector[0][0])
    print("Yesterday real: ", df['Close'].iloc[-1])


    print("Predicted relative change: ", round((output_vector[0][0] / df['Close'].iloc[-2] - 1) * 100, 5), " %") 
    print("Real relative change: ", round((df['Close'].iloc[-1] / df['Close'].iloc[-2] - 1) * 100, 5), " %")

    f = open('neural_net/predictions_naive.csv', 'a', newline='')
    writer = csv.writer(f)
    ...
    row.append(output_vector[0][0])
    row.append(df['Close'].iloc[-1])
    row.append(round((output_vector[0][0] / df['Close'].iloc[-2] - 1) * 100, 5))
    row.append(round((df['Close'].iloc[-1] / df['Close'].iloc[-2] - 1) * 100, 5))
